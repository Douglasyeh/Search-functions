window.onload = function(){
    ...
    for(var i = 0; i < tag.length; i++){
        tag[i].addEventListener("click", tagClick);
    }

    function tagClick(){
        searchArr = searchData[this.innerText];
        search.value = this.innerText;
        DOMrender(searchArr);
    }

    search.addEventListener("keyup", function(){
        searchArr = searchData[this.value];
    ...
    function DOMrender(){
        ...
        searchArr.forEach(function(obj) {
            html+= '<div class="item">';
            html+= '<a href="javascript:;" target="_blank" class="title">' + obj.title +'</a>';
            html+= '<a href="'+ obj.link+'" target="_blank" class="link">' + obj.link + '</a>';
            html+= '<p>' + obj.text +'</p>';
            html+= '</div>';
        })
